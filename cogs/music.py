...
class YTDLSource(discord.PCMVolumeTransformer):
    ...
    def __init__(self, ctx: commands.Context, source: discord.FFmpegPCMAudio, *, data: dict, volume: float = 1.0):
        ...
        self.secs = int(data.get('duration'))
        self.duration = self.parse_duration(int(data.get('duration')))
        self.duration2 = self.parse_duration2(int(data.get('duration')))
    ...
    @staticmethod
    def parse_duration2(duration: int):
      dur = time.strftime('%H:%M:%S', time.gmtime(duration))
      return dur
...
class Song:
    ...
    def create_embed(self):
        embed = (discord.Embed(color=000000, description='**[{0.source.title}]({0.source.url})**'.format(self))
                 ...
                 .set_footer(text=f"Requested By {self.requester} ")
...
class Music(commands.Cog):
    ...
    async def _join(self, ctx: commands.Context):
        ...
        await ctx.send(f">>> Attempting to join VC...\nClient Latency : `{round((self.bot.latency * 1000))}ms`")
        channel = ctx.channel
        ...
        await ctx.send(f"> Successfully Connected To `{destination}` !")
    ...
    async def _summon(self, ctx: commands.Context, *, channel: discord.VoiceChannel = None):
        ...
        if not channel and not ctx.author.voice:
            raise VoiceError('You are neither connected to a voice channel nor have specified a channel to join.')
    ...
    async def _leave(self, ctx: commands.Context):
        ...
        await ctx.send(f"> Successfully Disconnected From `{ctx.author.voice.channel}` !")

    @client.command(name='volume', aliases=["vol"])
    async def _volume(self, ctx: commands.Context, *, percentage: float):
    ...
    @client.command(name='now', aliases=['current', 'playing', 'nowplaying'])
    async def _now(self, ctx: commands.Context):
        """Displays the currently playing song."""
        res = ctx.voice_state.current.create_embed()
    ...
    async def _resume(self, ctx: commands.Context):
        ...
        if ctx.voice_state.voice.is_paused():
            ...
            await ctx.reply("**:play_pause: Resumed**")
    ...
    async def _stop(self, ctx: commands.Context):
        ...
        if ctx.voice_state.is_playing:
            ...
            await ctx.reply("⏹ Stopped current track and cleared queue")
    ...
    async def _skip(self, ctx: commands.Context):
        ...
        if voter == ctx.voice_state.current.requester:
            ...
            await ctx.send("⏭ Instantly Skipped To Next Track As Skipped By Requester")
            ...
            if total_votes >= 3:
                ...
                await ctx.send('Skip vote added, currently at **{}/3** votes to skip.'.format(total_votes))

        else:
            await ctx.send('You have already voted to skip this song.')

    @client.command(name='forceskip', aliases=["fs"])
    async def _forceskip(self, ctx: commands.Context):
      """Forcefully skips to the next song without a voting system."""
      ctx.voice_state.skip()
      await ctx.reply("⏭ Forcefully skipped To Next Track")
    ...
    async def _queue(self, ctx: commands.Context, *, page: int = 1):
        ...
        if len(ctx.voice_state.songs) == 0:
            return await ctx.send('There are no songs queued after the current track.')
        ...
        for i, song in enumerate(ctx.voice_state.songs[start:end], start=start):
            queue += '**{0}.** [**{1.source.title}**]({1.source.url}) | `{1.source.duration2}` | `Requested by {1.source.requester}`\n\n'.format(i + 1, song)
        
        try:
          
          def pdu(duration: int):
            minutes, seconds = divmod(duration, 60)
            hours, minutes = divmod(minutes, 60)
            days, hours = divmod(hours, 24)
        ...
        res= ctx.voice_state.current.return_source()
        source=res
       
        embed = (discord.Embed(description='`NOW PLAYING`:\n**[{}]({})**\n\n**Total Queue Duration : {}**\n\n**{} tracks behind in queue:**\n{}\n'.format(source.title, source.url, ndu, len(ctx.voice_state.songs), queue))
                 .set_author(name=f"Track Queue For {ctx.guild}", icon_url=ctx.guild.icon_url)
                 .set_footer(text='Viewing page {}/{}'.format(page, pages)))
        await ctx.send(embed=embed)
        

    @client.command(name="lyrics", aliases=["lyr", "lyric"])
    async def _lyrics(self, ctx: commands.Context, *, search="never gonna give you up"):
    ...
    async def _loop(self, ctx: commands.Context):
        ...
        await ctx.send(f":infinity: Loop Set To: {bool(ctx.voice_state.loop)}")
        await ctx.message.add_reaction('✅')
    ...
    async def _play(self, ctx: commands.Context, *, search: str):
        ...
        await ctx.send(f"Seaching For `{search}` :mag: ")
        async with ctx.typing():
            try:
